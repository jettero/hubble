...
@hubblestack.utils.decorators.path.which("traceroute")
def traceroute(host):
    """
    ...
    if __utils__["platform.is_sunos"]() or __utils__["platform.is_aix"]():
        ...
        version_out = __mods__["cmd.run"]("traceroute --version")
        try:
...
@hubblestack.utils.decorators.path.which("dig")
def dig(host):
    """
...
@hubblestack.utils.decorators.path.which("arp")
def arp():
    """
...
hwaddr = hubblestack.utils.functools.alias_function(hw_addr, "hwaddr")
...
ipaddrs = hubblestack.utils.functools.alias_function(ip_addrs, "ipaddrs")
...
ipaddrs6 = hubblestack.utils.functools.alias_function(ip_addrs6, "ipaddrs6")
...
def mod_hostname(hostname):
    ...
    if hostname_cmd.endswith("hostnamectl"):
        result = __mods__["cmd.run_all"]("{0} status".format(hostname_cmd))
        if 0 == result["retcode"]:
            out = result["stdout"]
            for line in out.splitlines():
                line = line.split(":")
            ...
            o_hostname = __mods__["network.get_hostname"]()
        ...
        o_hostname = __mods__["cmd.run"](check_hostname_cmd).split(" ")[-1]
    ...
    if hostname_cmd.endswith("hostnamectl"):
        ...
        __mods__["cmd.run"]("{0} {1}".format(hostname_cmd, hostname))
    else:
        __mods__["cmd.run"]("{0} -S {1}".format(uname_cmd, hostname.split(".")[0]))
...
def fqdns():
    ...
    addresses = hubblestack.utils.network.ip_addrs(
        include_loopback=False, interface_data=hubblestack.utils.network.interfaces()
    )
    addresses.extend(
        hubblestack.utils.network.ip_addrs6(
            include_loopback=False, interface_data=hubblestack.utils.network.interfaces()
        )
    )
    ...
    log.debug("Elapsed time getting FQDNs: %d seconds", elapsed)
