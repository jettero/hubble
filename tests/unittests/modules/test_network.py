...
class NetworkTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def setup_loader_modules(self):
        opts = hubblestack.config.DEFAULT_MINION_OPTS.copy()
        utils = hubblestack.loader.utils(
    ...
    def test_ping(self):
        ...
        with patch.dict(
            ...
            with patch.dict(network.__mods__, {"cmd.run_all": mock_all}):
                self.assertFalse(network.ping("host", return_boolean=True))

                self.assertTrue(network.ping("host", return_boolean=True))

            with patch.dict(network.__mods__, {"cmd.run": MagicMock(return_value="A")}):
    ...
    def test_traceroute(self):
        ...
        with patch("salt.utils.path.which", MagicMock(return_value="traceroute")):
            with patch.dict(network.__mods__, {"cmd.run": MagicMock(return_value="")}):
            ...
            with patch.dict(
                ...
                with patch.dict(
                    network.__mods__, {"cmd.run": MagicMock(return_value="")}
                ):
                    self.assertListEqual(network.traceroute("gentoo.org"), [])
    ...
    def test_mod_hostname(self):
        ...
        with patch.dict(
            ...
            network.__mods__, {"cmd.run": MagicMock(return_value=None)}
        ), patch.dict(
    ...
    def test_get_route(self):
        ...
        with patch.dict(network.__grains__, {"kernel": "Linux"}):
            with patch.dict(network.__mods__, {"cmd.run": mock_iproute}):
                expected = {
                    "interface": "eth0",
                    "source": "10.10.10.10",
                    "destination": "8.8.8.8",
